...
def shockit():
  ...
  if verify_url(websiteUrl):
    ...
    if app.config['USE_SENTRY']:
      sentry.captureMessage('{ip} requested exploit of {url} with commonVulnerableRoutes: {cvr}'.format(ip=request.environ['REMOTE_ADDR'], url=websiteUrl, cvr=commonVulnerableRoutes))
